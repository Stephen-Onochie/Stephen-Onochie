...
| **Dorm OS** | Interactive 3D diorama of Stephen's Wiley Hall dorm room (`/apps/dorm`). The engine is `components/dorm/dorm-room.js` — a verbatim vanilla Three.js custom element (`<dorm-room>`) ported from a prototype; do not edit it. React chrome in `components/dorm/`: Room OS panel (day/night, string lights, computer, TV, curtains, fans, auto spin, view/edit furniture dragging); night mode dims the module content independently of IVEN dark mode. `three` pinned exactly at 0.158.0 and assigned to `window.THREE`; `DormStage` must only be imported via `next/dynamic` `ssr: false` (the engine defines a custom element at module scope). Engine treats unset `_autoRotate` as ON — push `false` on mount. No DB tables. |
